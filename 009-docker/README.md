...
Команда ping была запущена 2 раза и было отправлено 14 запросов. 
...
#### 2) Запуск контейнера
```bash 
docker run -it --name mynode -e NAME=Stanislav -e SURNAME=Ivanov node:15.14
```
```
...
```
#### 4) Остановка контейнера Ctrl+C (2)
...
What's Next?
  View a summary of image vulnerabilities and recommendations → docker scout quickview node:15.14
```
#### 2) Запуск контейнера с привязкой к папке на хосте
```bash
docker run -it --name first_node -v %cd%\data:/var/first/data node:15.14
```
```
Welcome to Node.js v15.14.0.
Type ".help" for more information.
>
```
#### 3) Запуск контейнера с привязкой к папке на хосте
```bash
docker run -it --name second_node -v C:\Users\Станислав\Desktop\Project\ndse-homeworks\009-docker\data:/var/first/data node:15.14
```
```
Welcome to Node.js v15.14.0.
Type ".help" for more information.
>
```
#### 4) Подключение к контейнеру и создание файла
```bash
docker exec -it first_node /bin/sh
cd /var/first/data
touch emptyfile.txt
```
#### 5) Создание на хосте text1.txt
...
text.txt  text1.txt
```
#### 7) Остановка 2х контейнеров 

Ctrl + c (4)

#### 8) Удаление контейнеров 
```bash
